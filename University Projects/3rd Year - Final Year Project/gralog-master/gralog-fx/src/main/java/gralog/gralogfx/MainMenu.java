...
public class MainMenu {
    ...
    public static class Handlers {
        ...
        MenuStringAction onNew, onGenerate, onRunAlgorithm, onRunVisualize; // Added onRunVisualize matching changes in MainWindow - Ethan
    }
    ...
    // New Menu for Visualized Algorithms - Ethan
    private Menu menuVisualize;
...
//            createMenuItem("Load spec'd program", handlers.onLoadPluginFromSpecifiedFilepath),
    ...
    // New function to allow Visualize menu to be added to Menu tab - Ethan
    private Menu createVisualizeMenu() {
        menuVisualize = new Menu("Visualize");
        return menuVisualize;
    }
    ...
    MainMenu(Handlers handlers) {
        ...
        menu.getMenus().addAll(
            ...
            createAlgorithmMenu(),
            createVisualizeMenu(), // Adds Visualize menu to Menu tab - Ethan
            createHelpMenu());
    ...
    public void update() {
        ...
        updateGenerators();
        updateAlgorithms();
        updateVisualize(); // Updates Visualize menu - Ethan
    }
    ...
    // New function that adds Algorithms that can be visualized to Visualize menu - Ethan
    private void updateVisualize() {
        menuVisualize.getItems().clear();

        if (currentStructure == null) {
            return;
        }

        for (String str : AlgorithmManager.getVisualizeAlgorithms(currentStructure.getClass()))
            menuVisualize.getItems().add(
                createMenuItem(str, handlers.onRunVisualize)); // Sets each menu tab to run 'onRunVisualize' on selection
    }
