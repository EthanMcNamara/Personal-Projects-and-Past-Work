...
public abstract class Algorithm {
    ...
    public Object doRun(Structure structure, AlgorithmParameters params,
                        Set<Object> selection, ProgressHandler onprogress, boolean isVis) throws Exception { // Added isVis as parameter - Ethan
        Object algoResult = null;
        Method[] methods = this.getClass().getMethods();
        for (Method method : methods) {
            if(isVis && !method.getName().equals("runVis")) { // If isVis is true, call 'runVis' - Ethan
                continue;
            }
            if(!isVis && !method.getName().equals("run")) // Otherwise, call 'run' - Ethan
                continue;
