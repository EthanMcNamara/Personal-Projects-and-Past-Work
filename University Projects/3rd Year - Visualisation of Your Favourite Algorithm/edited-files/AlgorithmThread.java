...
public class AlgorithmThread extends Thread {
    ...
    boolean isVis; // New variable to indicate if Algorithm is being Visualized or not - Ethan
    ...
    public AlgorithmThread(Algorithm algo, Structure structure,
        ...
        ProgressHandler onprogress, boolean isVis) { // Added isVis as parameter to AlgorithmThread - Ethan
        setDaemon(true);
        ...
        this.isVis = isVis; // Sets isVis to input value - Ethan
    }
    ...
    public void run() {
        try {
            result = this.algo.doRun(structure, params, selection, onprogress, isVis); // Calls doRun, passing in isVis - Ethan
        } catch (Exception ex) {
            this.exception = ex;
        }
        if (handler != null)
