...
public final class AlgorithmManager {
    ...
    // New function that only adds Algorithms that can be Visualized - Ethan
    public static List<String> getVisualizeAlgorithms(Class<?> forClass) {
        List<String> result = new ArrayList<>();

        for (Class i = forClass; i != null; i = i.getSuperclass())
            if (ALGORITHM_NAMES.containsKey(i)) {
                for(String s : ALGORITHM_NAMES.get(i).keySet()) {
                    if(s.equals("Minimum Spanning Tree (Prim's)") || s.equals("Minimum Spanning Tree (Kruskal's)") ||
                    s.equals("Floyd-Warshall Shortest Path")) { // Lazy but can't find a better way! - Ethan
                        result.add(s);
                    }
                }
            }
